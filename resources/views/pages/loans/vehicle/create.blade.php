        ...
        <form action="{{ route('vehicleLoans.store') }}" method="POST" novalidate>
            ...
            <div class="flex flex-col justify-between gap-3 md:flex-row">
                ...
                <div class="mb-6 hidden">
                    ...
                    <input type="text" id="department_id" name="department_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="" readonly value="{{ Auth::user()->department->id }}">
                    @error('department_id')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <input type="submit" id="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" value="Submit">
        </form>
    </x-slot>
</x-card>
...
<script>
    ...
    const vehicleDetail = document.getElementById('vehicle_detail')
    ...
    vehicleDropdown.addEventListener('change', async function() {
        ...
        if(selectedVeh !== null) {
            ...
            vehicleDetail.classList.toggle('hidden')
        }
    })
