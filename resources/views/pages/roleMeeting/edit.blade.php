...
<x-card>
    <x-slot name="header">
        <h1 class="pb-2 mb-6 text-3xl border-b-4 border-slate-800 w-fit dark:text-slate-200 dark:border-slate-200">Edit Role For Meeting</h1>
    </x-slot>
    <x-slot name="content">
        <form action="{{ route('role-meeting.update', ['role_meeting' => $role->id]) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
            <div class="mb-6">
            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Position Name</label>
            ...
            <div class="mb-6">
                ...
                <select id="status" name="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    @foreach ($status as $s)
                        <option value="{{ $s }}" {{ $s === $role->status ? 'selected' : '' }}>{{ $s }}</option>
                    @endforeach
                </select>
            </div>
