...
<x-card>
    <x-slot name="header">
        <h1 class="pb-2 mb-6 text-3xl border-b-4 border-slate-800 w-fit dark:text-slate-200 dark:border-slate-200">Edit Role For Asset</h1>
    </x-slot>
    <x-slot name="content">
        <form action="{{ route('role-asset.update', ['role_asset' => $role->id]) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
            <div class="mb-6">
