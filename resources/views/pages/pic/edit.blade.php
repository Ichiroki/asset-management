...
@section('main')
    <x-card>
        ...
        <x-slot name="content">
            <form action="{{ route('vehicle-pic.update', ['vehicle_pic' => $pic->id]) }}" method="POST" novalidate>
                @csrf
                @method('put')
                <div class="mb-6">
                    ...
                    <select id="vehicle_id" name="vehicle_id"
                        ...
                        @foreach ($vehicles as $vehicle)
                            <option
                            value="{{ $vehicle->id }}"
                             @if($vehicle->id == $pic->vehicle_id) selected @endif>{{ $vehicle->type }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-6">
                    ...
                    <div class="flex">
                        <div class="mr-4">
                            <input type="radio" id="pic_user" name="pic_checkbox" class="mr-2" value="1" @if($pic->user_id) checked @endif>
                            <label for="pic_user" class="text-sm font-medium text-gray-900 dark:text-white">User</label>
                        </div>
                        <div>
                            <input type="radio" id="pic_department" name="pic_checkbox" class="mr-2" value="2" @if($pic->department_id) checked @endif>
                            <label for="pic_department"
                                class="text-sm font-medium text-gray-900 dark:text-white">Department</label>
                        </div>
                    </div>
                </div>
                <div id="user_select" class="mb-6 @if($pic->user_id) hidden @endif">
                    ...
                    <select id="user_id" name="pic_user"
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}"
                                @if($user->id == $pic->id) selected @endif>{{ $user->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div id="department_select" class="mb-6 @if($pic->department_id) hidden @endif">
