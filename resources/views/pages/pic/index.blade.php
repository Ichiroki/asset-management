            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            {{ $pic->vehicle->number_plates }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $pic->vehicle->capacity }}
                        </td>
                            @if($pic->user !== null)
                            <td class="px-6 py-4" id="pic">
                                {{ $pic->user->name }}
                            </td>
                            @else
                            <td class="px-6 py-4" id="pic"></td>
                            @endif
                            @if($pic->department !== null)
                            <td class="px-6 py-4" id="pic">
                                {{ $pic->department->name }}
                            </td>
                            @else
                            ...
                            <div class="flex items-center">
                                <a href="{{ route('vehicle-pic.edit', ['vehicle_pic' => $pic->id]) }}" class="px-3 py-2 mb-2 mr-2 text-sm font-medium text-center text-blue-700 transition border border-blue-700 rounded-lg hover:text-white hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-600 dark:focus:ring-blue-800">Edit</a>
                                <button data-modal-target="popup-modal-{{ $pic->id }}" data-modal-toggle="popup-modal-{{ $pic->id }}" class="px-3 py-2 mb-2 mr-2 text-sm font-medium text-center transition border rounded-lg text-rose-700 hover:text-white border-rose-700 hover:bg-rose-800 focus:ring-4 focus:outline-none focus:ring-rose-300 dark:border-rose-500 dark:text-rose-500 dark:hover:text-white dark:hover:bg-rose-600 dark:focus:ring-rose-800" type="button">
                                    Delete
                                </button>
                                <x-modal-box value="{{ $pic->id }}">
                                    <x-slot name="logo">
                                    ...
                                    <form action="{{ route('vehicle-pic.destroy', ['vehicle_pic' => $pic->id]) }}" class="inline" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                            Yes, I'm sure
                                        </button>
                                        <button data-modal-hide="popup-modal-{{ $pic->id }}" type="button" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">No, cancel</button>
                                    </form>
                                </x-modal-box>
                            </div>
                        </td>
